...
app.get('/', async (req, res) => {
  ...
  const recentResultPromise = pool.query(
  ...
  const tabsResultPromise = pool.query(stmt, ["TABS"])
    .then((rows) => {
      return rows[0].count;
    });
  // Get the total number of "SPACES" votes.
  const spacesResultPromise = pool.query(stmt, ["SPACES"])
    .then((rows) => {
      return rows[0].count;
    });
...
try {
  ...
  res
    .status(500)
    .send('Unable to successfully cast vote! Please check the application logs for more details.')
    .end();
  // [END_EXCLUDE]
}
