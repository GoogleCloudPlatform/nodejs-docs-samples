...
test(`should inspect a string`, async (t) => {
  ...
  t.regex(output, /"name": "US_MALE_NAME"/);
  t.regex(output, /"quote": "Gary"/);
  t.regex(output, /"name": "EMAIL_ADDRESS"/);
  t.regex(output, /"quote": "gary@example\.com"/);
});
...
test(`should inspect a local text file`, async (t) => {
  ...
  t.regex(output, /"quote": "\(123\) 456-7890"/);
  t.regex(output, /"name": "EMAIL_ADDRESS"/);
  t.regex(output, /"quote": "gary@somedomain\.com"/);
});
...
test(`should accept try limits for inspecting GCS files`, async (t) => {
...
test(`should inspect Datastore`, async (t) => {
  ...
  t.regex(output, /"name": "US_MALE_NAME"/);
  t.regex(output, /"name": "US_FEMALE_NAME"/);
});

test(`should accept try limits for inspecting Datastore`, async (t) => {
...
test(`should report Datastore file handling errors`, async (t) => {
...
test(`should have a maxFindings option`, async (t) => {
  const promiseA = runAsync(`${cmd} string "My name is Gary and my phone number is (123) 456-7890." -f 1`);
  const promiseB = runAsync(`${cmd} string "My name is Gary and my phone number is (123) 456-7890." -f 2`);

  const outputA = await promiseA;
  t.not(outputA.includes('PHONE_NUMBER'), outputA.includes('US_MALE_NAME')); // Exactly one of these should be included

  const outputB = await promiseB;
  t.regex(outputB, /PHONE_NUMBER/);
  t.regex(outputB, /US_MALE_NAME/);
});

test(`should have an option to include quotes`, async (t) => {
  const promiseA = runAsync(`${cmd} string "My name is Gary and my phone number is (123) 456-7890." -q false`);
  const promiseB = runAsync(`${cmd} string "My name is Gary and my phone number is (123) 456-7890."`);

  const outputA = await promiseA;
  t.truthy(outputA);
  t.notRegex(outputA, /Gary/);

  const outputB = await promiseB;
  t.regex(outputB, /Gary/);
});

test(`should have an option to filter results by infoType`, async (t) => {
  const promiseA = runAsync(`${cmd} string "My name is Gary and my phone number is (123) 456-7890."`);
  const promiseB = runAsync(`${cmd} string "My name is Gary and my phone number is (123) 456-7890." -t PHONE_NUMBER`);

  const outputA = await promiseA;
  t.regex(outputA, /US_MALE_NAME/);
  t.regex(outputA, /PHONE_NUMBER/);

  const outputB = await promiseB;
  t.notRegex(outputB, /US_MALE_NAME/);
  t.regex(outputB, /PHONE_NUMBER/);
});

test(`should have an option for custom auth tokens`, async (t) => {
  const output = await runAsync(`${cmd} string "My name is Gary and my phone number is (123) 456-7890." -a foo`);
  t.regex(output, /Error in inspectString/);
