...
test.serial(`processImage fails without a name`, async t => {
  ...
  const err = await t.throws(
    getSample().program.processImage({data: {bucket: bucketName}})
  );
  t.deepEqual(err, error);
});

test.serial(
  `processImage processes an image with Node 6 arguments`,
  async t => {
    const event = {
      data: {
        bucket: bucketName,
...
test.serial(
  `processImage processes an image with Node 8 arguments`,
  async t => {
    const data = {
      bucket: bucketName,
      name: filename,
...
test.serial(
  `translateText translates and publishes text with Node 6 arguments`,
  async t => {
    const event = {
      data: {
        data: Buffer.from(
...
test.serial(
  `translateText translates and publishes text with Node 8 arguments`,
  async t => {
    const data = {
      data: Buffer.from(
        JSON.stringify({
...
test.serial(
  `saveResult translates and publishes text with Node 6 arguments`,
  async t => {
    const event = {
      data: {
...
test.serial(
  `saveResult translates and publishes text with Node 8 arguments`,
  async t => {
    const data = {
      data: Buffer.from(JSON.stringify({text, filename, lang})).toString(
        `base64`
...
test.serial(
  `saveResult translates and publishes text with dot in filename`,
  async t => {
    const event = {
      data: {
        data: Buffer.from(
