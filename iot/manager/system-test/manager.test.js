...
before(async () => {
  childProcess.exec(installDeps, `${cwd}/../mqtt_example`);
  assert(
...
after(async () => {
  ...
  childProcess.exec(`${cmd} unbindAllDevices ${registryName}`, cwd);
  childProcess.exec(`${cmd} clearRegistry ${registryName}`, cwd);
