...
describe('Remove fulfillment', () => {
  ...
  before(async () => {
    stdout = execSync(
      `node interactive-tutorials/product/remove-fulfillment-places.js ${productId}`,
      {cwd}
    );
  });
  ...
  const checkUpdatedProduct = updatedProduct => {
    ...
    expect(item.placeIds)
      .to.be.an('array')
      .that.includes('store3', 'store2')
      .but.not.include('store1');
  };

  it('should check that fulfillment removed correctly', async () => {
    const regex = new RegExp('Updated product with current time: .*\\n', 'g');
    assert.match(stdout, regex);
    const string = stdout
      .match(regex)
      .toString()
      .replace('Updated product with current time: ', '');
