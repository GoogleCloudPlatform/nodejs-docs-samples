...
const service_name = process.env.CLOUD_RUN_SERVICE_NAME || require('../package').name;
...
describe('Logging', () => {
  ...
  describe("Live Service", () => {
    it ('can be reached by an HTTP request', async () => {
      ...
      if (!url) {
        throw Error('"$CLOUD_RUN_SERVICE_URL" environment variable is required. For example: https://service-x8xabcdefg-uc.a.run.app.')
      }
      await request(url);
    });
    ...
    it ('generates Stackdriver logs', async() => {
      ...
      let filter = `resource.labels.service_name="${service_name}" timestamp>="${dateMinutesAgo((new Date), 5)}"`;
  ...
  describe('Structured Logging', () => {
    ...
    it('uses "severity" property as entry severity', () => {
      assert(sampleLog.metadata.severity, "'severity' lifted to official property");
    });

    it('uses "trace" property as valid entry trace ID', () => {
      assert(sampleLog.metadata.trace, "'trace' lifted to official property");
      assert(sampleLog.metadata.trace == requestLog.metadata.trace, "`trace` property matches request log trace ID");
    });
  });

  describe('Cloud Run Log Metadata', () => {
    it('has expected label properties', () => {
      let labels = sampleLog.metadata.labels;
      let resource = sampleLog.metadata.resource;

      assert(labels.instanceId, "'labels.instanceId' found in the log entry");
      assert(resource.labels.configuration_name, "'resource.labels.configuration_name' found in the log entry");
      assert(resource.labels.location, "'resource.labels.location' found in the log entry");
      assert(resource.labels.project_id, "'resource.labels.project_id' found in the log entry");
      assert(resource.labels.revision_name, "'resource.labels.revision_name' found in the log entry");
      assert(resource.labels.service_name, "'resource.labels.service_name' found in the log entry");
    });
  });
