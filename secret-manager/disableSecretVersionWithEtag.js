...
async function main(name = 'projects/my-project/secrets/my-secret/versions/1') {
  ...
  async function disableSecretVersionWithEtag() {
    const [version] = await client.getSecretVersion({
      name: name,
    });
    const [disabledVersion] = await client.disableSecretVersion({
      name: name,
      etag: version.etag,
    });

    console.info(`Disabled ${disabledVersion.name}`);
  }

  disableSecretVersionWithEtag();
