...
async function main(projectId, locationId, secretId, version) {
  ...
  async function enableRegionalSecretVersionWithEtag() {
    const [version] = await client.getSecretVersion({
      name: name,
    });
    const [enableRegionalVersion] = await client.enableSecretVersion({
      name: name,
      etag: version.etag,
    });

    console.info(`Enabled ${enableRegionalVersion.name}`);
  }
