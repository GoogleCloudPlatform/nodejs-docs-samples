...
async function deleteCustomModule(customModuleId) {
    ...
    await client.deleteEventThreatDetectionCustomModule(deleteEventThreatDetectionCustomModuleRequest);
}
...
describe('Event Threat Detection Custom module', async () => {
    ...
    before(async () => {
                ...
                metadata: {
                    structValue: {
                        fields: {
                            ...
                            description: { stringValue: 'add your description' },
        ...
        try {
            const [response] = await client.createEventThreatDetectionCustomModule(createEventThreatDetectionCustomModuleRequest);
            // extracts the custom module ID from the full name
    ...
    it('should create the event threat detection custom module', done => {
        const output = exec(`node management_api/createEventThreatDetectionCustomModule.js ${data.orgId} ${data.customModuleName}`);
    ...
    it('should get the event threat detection custom module', done => {
        const output = exec(`node management_api/getEventThreatDetectionCustomModule.js ${data.orgId} ${data.customModuleId}`);
        assert(output.includes(data.orgId));
    ...
    it('should list all the event threat detection custom module', done => {
        const output = exec(`node management_api/listEventThreatDetectionCustomModules.js ${data.orgId}`);
        assert(output.includes(data.orgId));
        assert.notMatch(output, /undefined/);
        done();
    });
    ...
    it('should delete the event threat detection custom module', done => {
        const output = exec(`node management_api/deleteEventThreatDetectionCustomModule.js ${data.orgId} ${data.customModuleId}`);
        assert.match(output, /EventThreatDetectionCustomModule deleted successfully/);
        assert.notMatch(output, /undefined/);
        done();
    });
});