...
const customModuleDisplayName = 'security_health_analytics_test' + uuidv1().replace(/-/g, '_');
...
describe('security health analytics custom module', async () => {
    ...
    before(async () => {
        ...
        try {
            const [createResponse] = await client.createSecurityHealthAnalyticsCustomModule({
                parent: parent,
            ...
            console.log('SecurityHealthAnalyticsCustomModule created : %j', createResponse);
        } catch (error) {
            console.error('Error creating SecurityHealthAnalyticsCustomModule:', error);
        }
    ...
    after(async () => {
        ...
        const [listResponse] = await client.listSecurityHealthAnalyticsCustomModules({
            ...
            try {
                if (module.displayName === customModuleDisplayName) {
                    ...
                    if (customModuleId) {
                        ...
                        console.log(`Custome Module ${customModuleDisplayName} deleted successfully.`);
                    }
                }
            } catch (error) {
                console.error('Error deleting SecurityHealthAnalyticsCustomModule:', error);
            }
        }
    ...
    it('create security health analytics custom module', done => {
        ...
        assert.match(output, /Security Health Analytics Custom Module creation succeeded/);
    ...
    it('update security health analytics custom module', done => {
        ...
        assert.match(output, /Security Health Analytics Custom Module update succeeded/);
        assert.notMatch(output, /undefined/);
        done();
    });
    ...
    it('get security health analytics custom module', done => {
        ...
        assert.include(output, data.customModuleName);
        assert.match(output, /Security Health Analytics Custom Module get succeeded/);
        assert.notMatch(output, /undefined/);
        done();
    });
    ...
    it('get effective security health analytics custom module', done => {
        ...
        assert.include(output, data.customModuleName);
        assert.match(output, /Security Health Analytics Custom Module get effective succeeded/);
        assert.notMatch(output, /undefined/);
        done();
    });
