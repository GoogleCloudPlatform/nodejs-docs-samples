...
test.serial(`should create an example database`, async (t) => {
  const output = await runAsync(`${schemaCmd} createDatabase "${INSTANCE_ID}" "${DATABASE_ID}"`, cwd);
...
test.serial(`should insert rows into an example table`, async (t) => {
  let output = await runAsync(`${crudCmd} insert ${INSTANCE_ID} ${DATABASE_ID}`, cwd);
  t.true(output.includes(`Inserted data.`));
});

// query_data
test.serial(`should query an example table and return matching rows`, async (t) => {
  const output = await runAsync(`${crudCmd} query ${INSTANCE_ID} ${DATABASE_ID}`, cwd);
  t.true(output.includes(`SingerId: 1, AlbumId: 1, AlbumTitle: Go, Go, Go`));
});

// read_data
test.serial(`should read an example table`, async (t) => {
  const output = await runAsync(`${crudCmd} read ${INSTANCE_ID} ${DATABASE_ID}`, cwd);
  t.true(output.includes(`SingerId: 1, AlbumId: 1, AlbumTitle: Go, Go, Go`));
});

// add_column
test.serial(`should add a column to a table`, async (t) => {
  const output = await runAsync(`${schemaCmd} addColumn ${INSTANCE_ID} ${DATABASE_ID}`, cwd);
...
test.serial(`should update existing rows in an example table`, async (t) => {
  let output = await runAsync(`${crudCmd} update ${INSTANCE_ID} ${DATABASE_ID}`, cwd);
...
test.serial(`should query an example table with an additional column and return matching rows`, async (t) => {
  const output = await runAsync(`${schemaCmd} queryNewColumn ${INSTANCE_ID} ${DATABASE_ID}`, cwd);
...
test.serial(`should create an index in an example table`, async (t) => {
  let output = await runAsync(`${indexingCmd} createIndex ${INSTANCE_ID} ${DATABASE_ID}`, cwd);
  t.true(output.includes(`Waiting for operation to complete...`));
...
test.serial(`should create a storing index in an example table`, async (t) => {
  const output = await runAsync(`${indexingCmd} createStoringIndex ${INSTANCE_ID} ${DATABASE_ID}`, cwd);
  t.true(output.includes(`Waiting for operation to complete...`));
...
test.serial(`should query an example table with an index and return matching rows`, async (t) => {
  const output = await runAsync(`${indexingCmd} queryIndex ${INSTANCE_ID} ${DATABASE_ID}`, cwd);
...
test.serial(`should read an example table with an index`, async (t) => {
  const output = await runAsync(`${indexingCmd} readIndex ${INSTANCE_ID} ${DATABASE_ID}`, cwd);
...
test.serial(`should read an example table with a storing index`, async (t) => {
  const output = await runAsync(`${indexingCmd} readStoringIndex ${INSTANCE_ID} ${DATABASE_ID}`, cwd);
  t.true(output.includes(`AlbumId: 1, AlbumTitle: Go, Go, Go`));
});

// read_only_transaction
test.serial(`should read an example table using transactions`, async (t) => {
  const output = await runAsync(`${transactionCmd} readOnly ${INSTANCE_ID} ${DATABASE_ID}`, cwd);
  t.true(output.includes(`SingerId: 1, AlbumId: 1, AlbumTitle: Go, Go, Go`));
...
test.serial(`should read from and write to an example table using transactions`, async (t) => {
  let output = await runAsync(`${transactionCmd} readWrite ${INSTANCE_ID} ${DATABASE_ID}`, cwd);
  ...
  output = await runAsync(`${schemaCmd} queryNewColumn ${INSTANCE_ID} ${DATABASE_ID}`, cwd);
