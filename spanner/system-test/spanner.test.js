...
describe('Autogenerated Admin Clients', () => {
  ...
  describe('queues', () => {
    ...
    it('should create a database with a queue', async function () {
      try {
        const output = execSync(
          `${queueCreateDatabaseCmd} ${PROJECT_ID} ${INSTANCE_ID} ${QUEUE_DATABASE_ID}`
        );
        assert.match(
          output,
    ...
    it('should send a message to a queue using Mutation API', async function () {
      ...
      const output = execSync(
        `${queueSendMutationCmd} ${PROJECT_ID} ${INSTANCE_ID} ${QUEUE_DATABASE_ID}`
      );
      assert.match(
        output,
    ...
    it('should send a message to a queue using SQL API', async function () {
      ...
      const output = execSync(
        `${queueSendSqlCmd} ${PROJECT_ID} ${INSTANCE_ID} ${QUEUE_DATABASE_ID}`
      );
      assert.match(
        output,
    ...
    it('should schedule a message to a queue for future delivery using Mutation API', async function () {
      ...
      const output = execSync(
        `${queueSendMutationFutureCmd} ${PROJECT_ID} ${INSTANCE_ID} ${QUEUE_DATABASE_ID}`
      );
      assert.match(
        output,
        new RegExp(
    ...
    it('should schedule a message to a queue for future delivery using SQL API', async function () {
      ...
      const output = execSync(
        `${queueSendSqlFutureCmd} ${PROJECT_ID} ${INSTANCE_ID} ${QUEUE_DATABASE_ID}`
      );
      assert.match(
        output,
        new RegExp(
    ...
    it('should send and receive a message from a queue using SQL API', async function () {
      ...
      const output = execSync(
        `${queueSendAndReceiveSqlCmd} ${PROJECT_ID} ${INSTANCE_ID} ${QUEUE_DATABASE_ID}`
      );
      assert.match(
        output,
        new RegExp('Successfully sent 1 message to queue using SQL API')
    ...
    it('should acknowledge a message from a queue using Mutation API', async function () {
      ...
      const output = execSync(
        `${queueAckMutationCmd} ${PROJECT_ID} ${INSTANCE_ID} ${QUEUE_DATABASE_ID}`
      );
      assert.match(
        output,
        new RegExp(
    ...
    it('should acknowledge a message from a queue using SQL API with ASSERT_ROWS_MODIFIED', async function () {
      ...
      const output = execSync(
        `${queueAckSqlCmd} ${PROJECT_ID} ${INSTANCE_ID} ${QUEUE_DATABASE_ID}`
      );
      assert.match(
        output,
        new RegExp(
    ...
    it('should delete a message from a queue using SQL API', async function () {
      ...
      const output = execSync(
        `${queueDeleteSqlCmd} ${PROJECT_ID} ${INSTANCE_ID} ${QUEUE_DATABASE_ID}`
      );
      assert.match(
        output,
