...
const queueCreateDatabaseCmd = 'node queue-create-database.js';
const queueSendMutationCmd = 'node queue-send-with-mutation-api.js';
const queueSendSqlCmd = 'node queue-send-with-sql-api.js';
...
const QUEUE_DATABASE_ID = `test-queue-db-${CURRENT_TIME}`;
...
describe('Autogenerated Admin Clients', () => {
  ...
  after(async () => {
    ...
    if (!INSTANCE_ALREADY_EXISTS) {
      ...
      await Promise.all([
        ...
        safeDelete(
          instance.database(QUEUE_DATABASE_ID).delete(),
          'QUEUE_DATABASE_ID'
        ),
        safeDelete(instance.backup(BACKUP_ID).delete(GAX_OPTIONS), 'BACKUP_ID'),
        safeDelete(
  ...
  describe('queues', () => {
    ...
    it('should create a database with a queue', async function () {
      try {
        const output = execSync(
          `${queueCreateDatabaseCmd} ${INSTANCE_ID} ${QUEUE_DATABASE_ID} ${PROJECT_ID}`
        );
        assert.match(
          output,
          new RegExp('Created database .* with queue MyQueue')
        );
      } catch (err) {
        if (
          err.message.includes('UNIMPLEMENTED') ||
          err.message.includes('501') ||
          err.message.includes('Queue is not enabled') ||
          err.message.includes('CREATE QUEUE')
        ) {
    ...
    it('should send a message to a queue using Mutation API', async function () {
      if (!queueSupported) {
        this.skip();
      }
      const output = execSync(
        `${queueSendMutationCmd} ${INSTANCE_ID} ${QUEUE_DATABASE_ID} ${PROJECT_ID}`
      );
      assert.match(
        output,
        new RegExp('Successfully sent message to queue using Mutation API')
      );
    });

    it('should send a message to a queue using SQL API', async function () {
      if (!queueSupported) {
        this.skip();
      }
      const output = execSync(
        `${queueSendSqlCmd} ${INSTANCE_ID} ${QUEUE_DATABASE_ID} ${PROJECT_ID}`
      );
      assert.match(
        output,
        new RegExp('Successfully sent 1 message to queue using SQL API')
      );
    });

    it('should schedule a message to a queue for future delivery using Mutation API', async function () {
      if (!queueSupported) {
        this.skip();
      }
      const output = execSync(
        `${queueSendMutationFutureCmd} ${INSTANCE_ID} ${QUEUE_DATABASE_ID} ${PROJECT_ID}`
      );
      assert.match(
        output,
        new RegExp(
          'Successfully scheduled message to queue for .* using Mutation API'
        )
      );
    });

    it('should schedule a message to a queue for future delivery using SQL API', async function () {
      if (!queueSupported) {
        this.skip();
      }
      const output = execSync(
        `${queueSendSqlFutureCmd} ${INSTANCE_ID} ${QUEUE_DATABASE_ID} ${PROJECT_ID}`
      );
      assert.match(
        output,
        new RegExp(
          'Successfully scheduled 1 message to queue for 1 hour in future using SQL API'
        )
      );
    });

    it('should send and receive a message from a queue using SQL API', async function () {
      if (!queueSupported) {
        this.skip();
      }
      const output = execSync(
        `${queueSendAndReceiveSqlCmd} ${INSTANCE_ID} ${QUEUE_DATABASE_ID} ${PROJECT_ID}`
      );
      assert.match(
        output,
        new RegExp('Successfully sent 1 message to queue using SQL API')
      ...
      assert.match(
        output,
        new RegExp(
          'Received message: Id=5, Payload=Hello from Spanner Queue via SQL!'
        )
      );
    });

    it('should acknowledge a message from a queue using Mutation API', async function () {
      if (!queueSupported) {
        this.skip();
      }
      const output = execSync(
        `${queueAckMutationCmd} ${INSTANCE_ID} ${QUEUE_DATABASE_ID} ${PROJECT_ID}`
      );
      assert.match(
        output,
        new RegExp(
    ...
    it('should acknowledge a message from a queue using SQL API with ASSERT_ROWS_MODIFIED', async function () {
      if (!queueSupported) {
        this.skip();
      }
      const output = execSync(
        `${queueAckSqlCmd} ${INSTANCE_ID} ${QUEUE_DATABASE_ID} ${PROJECT_ID}`
      );
      assert.match(
        output,
        new RegExp(
          'Successfully acknowledged 1 message using SQL API with ASSERT_ROWS_MODIFIED'
        )
      );
    });

    it('should delete a message from a queue using SQL API', async function () {
      if (!queueSupported) {
        this.skip();
      }
      const output = execSync(
        `${queueDeleteSqlCmd} ${INSTANCE_ID} ${QUEUE_DATABASE_ID} ${PROJECT_ID}`
      );
      assert.match(
        output,
        new RegExp('Successfully deleted \\d+ message from queue using SQL API')
      );
    });
